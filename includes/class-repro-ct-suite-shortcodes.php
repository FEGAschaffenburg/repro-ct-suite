...
class Repro_CT_Suite_Shortcodes {
	...
	public function __construct() {
		...
		add_shortcode( 'repro_ct_suite_events', array( $this, 'render_events' ) ); // Alias für Kompatibilität
	...
	public function render_events( $atts ) {
		...
		Repro_CT_Suite_Logger::log( 'SHORTCODE EVENTS RETRIEVED - Count: ' . count($events), 'info' );
		...
		Repro_CT_Suite_Logger::log( 'SHORTCODE RENDER COMPLETE - Length: ' . strlen($output) . ' - Empty: ' . (empty($output) ? 'YES' : 'NO'), 'info' );
	...
	private function get_template_name( $view ) {
		...
		$templates = array(
			...
			'compact'      => 'events/list-compact.php',

			'medium'       => 'events/list-medium.php',

			'sidebar'      => 'events/list-sidebar.php',
