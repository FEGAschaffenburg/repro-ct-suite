...
class Repro_CT_Suite_Sync_Service {
	...
	private function fetch_all_events( array $args ): array {
		...
		// Client-seitige Filterung nach Datum (falls API-Filter nicht funktioniert)
		...
		foreach ( $events as $event ) {
			...
			if ( isset( $event['startDate'] ) ) {
				...
				$event_timestamp = strtotime( $event['startDate'] );

				if ( $event_timestamp >= $from_timestamp && $event_timestamp <= $to_timestamp ) {
		...
		$filtered_count = count( $filtered_events );

		Repro_CT_Suite_Logger::log( "Nach Zeitraum-Filter: {$filtered_count} / {$total} Events behalten" );
	...
	private function sync_calendar_events( int $external_calendar_id, array $args ): array {
	...
	private function sync_phase1_events( int $external_calendar_id, array $args, array &$imported_appointment_ids ) {
	...
	private function sync_phase2_appointments( int $external_calendar_id, array $args, array $imported_appointment_ids ) {
	...
	private function is_event_relevant_for_calendar( array $event, int $external_calendar_id ): bool {
	...
	private function is_appointment_relevant_for_calendar( array $appointment, int $external_calendar_id ): bool {
	...
	private function process_event( array $event, int $calendar_id ) {
	...
	private function extract_event_data( array $event, int $calendar_id ): array {
	...
	private function process_appointment( array $appointment, int $calendar_id ) {
	...
	private function extract_appointment_data( array $appointment, int $calendar_id ): array {
	...
	private function format_datetime_for_db( mixed $datetime ): string {
